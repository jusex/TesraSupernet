...
public:
	...
	h256 const& root() const { if (node(m_root).empty()) BOOST_THROW_EXCEPTION(BadRoot(m_root));  return m_root; }	
	...
	{
		_keyMask.erase(_k);
		if (_k == m_root && _k == c_shaNull)	
			return;
		descendList(RLP(node(_k)), _keyMask, _wasExt, _out, _indent);	
	}
...
{
	m_that = _db;
	m_trail.push_back({_db->node(_db->m_root), std::string(1, '\0'), 255});	
	next();
}
...
template <class DB> bytes GenericTrieDB<DB>::deleteAt(RLP const& _orig, NibbleSlice _k)
{
	...
	else
	{
		...
		else
		{
			...
			for (byte i = 0; i < 17; ++i)
				if (i == n)
					if (!deleteAtAux(r, _orig[i], _k.mid(1)))	
						return bytes();
					else {}
				else
					r << _orig[i];
...
template <class DB> bytes GenericTrieDB<DB>::cleve(RLP const& _orig, unsigned _s)
{
	...
	RLPStream top(2);
	top << hexPrefixEncode(k, false, 0, (int)_s);
