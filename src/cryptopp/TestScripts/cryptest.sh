...
# Clang {3.4|3.5|3.6} only advertises SSE2, http:
#   http:
...
if [[ "$IS_SOLARIS" -ne "0" ]]; then
	...
	if [[ (-e "/opt/solstudio12.2/bin/CC") ]]; then
		...
		# More workarounds... SunCC 5.11 only does SSSE3, http:
	...
	if [[ (-e "/opt/solarisstudio12.3/bin/CC") ]]; then
		...
		# More workarounds... SunCC 5.12 only does SSE4, http:
