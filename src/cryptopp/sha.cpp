...
INTEL_NOPREFIX
	...
	ASL(4)	
#endif
	AS2(	add		[AS_REG_7+0*4], ecx)	
	AS2(	add		[AS_REG_7+4*4], edi)	
	AS2(	mov		eax, B(0))
	AS2(	mov		ebx, C(0))
	AS2(	mov		ecx, D(0))
...
#if CRYPTOPP_BOOL_X32
	AS2(	lea		edi, [esp+8+8*8])		
	AS2(	lea		esi, [esp+8+20*8+8])	
#else
