...
#endif
{
	...
	if (IsIntel(cpuid))
	{
		...
		g_hasRDRAND = !!(cpuid1[2]  & RDRAND_FLAG);

		if (cpuid[0]  >= 7)
		{
			word32 cpuid3[4];
			if (CpuId(7, cpuid3))
				g_hasRDSEED = !!(cpuid3[1]  & RDSEED_FLAG);
		}
	}
	else if (IsAMD(cpuid))
	{
		...
		CpuId(0x01, cpuid);
		g_hasRDRAND = !!(cpuid[2]  & RDRAND_FLAG);
	...
	else if (IsVIA(cpuid))
	{
		...
		if (cpuid[0] >= 0xC0000001)
		{
			...
			g_hasPadlockRNG  = !!(cpuid[3]  & RNG_FLAGS);
			g_hasPadlockACE  = !!(cpuid[3]  & ACE_FLAGS);
			g_hasPadlockACE2 = !!(cpuid[3]  & ACE2_FLAGS);
			g_hasPadlockPHE  = !!(cpuid[3]  & PHE_FLAGS);
			g_hasPadlockPMM  = !!(cpuid[3]  & PMM_FLAGS);
		}
	}
...
# if defined(CRYPTOPP_MS_STYLE_INLINE_ASSEMBLY)
	...
	__try
	{
		...
		const uint64x2_t& t1 = vreinterpretq_u64_p128(r1);  
		const uint64x2_t& t2 = vreinterpretq_u64_p128(r2);  
		const uint64x2_t& t3 = vreinterpretq_u64_p128(r3);  
...
# else
	...
	else
	{
		...
		const uint64x2_t& t1 = (uint64x2_t)(r1);  
		const uint64x2_t& t2 = (uint64x2_t)(r2);  
		const uint64x2_t& t3 = (uint64x2_t)(r3);  
