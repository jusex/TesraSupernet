...
DWORD WINAPI WaitingThread(LPVOID lParam)
{
	...
	while (true)
	{
		...
		{
			CRYPTOPP_ASSERT(!"error in WaitingThread");	
...
void WaitObjectContainer::AddReadFd(int fd, CallStack const& callStack)	
{
...
void WaitObjectContainer::AddWriteFd(int fd, CallStack const& callStack) 
{
	CRYPTOPP_UNUSED(callStack);
...
bool Waitable::Wait(unsigned long milliseconds, CallStack const& callStack)
{
	...
	GetWaitObjects(container, callStack);	
