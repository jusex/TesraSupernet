...
bool TesraDGP::checkLimitSchedule(const std::vector<uint32_t> &defaultData, const std::vector<uint32_t> &checkData)
{
...
dev::eth::EVMSchedule TesraDGP::getGasSchedule(unsigned int blockHeight)
{
    clear();
...
uint64_t
TesraDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address &contract, std::vector<unsigned char> data)
{
    uint64_t value = 0;
...
uint32_t TesraDGP::getBlockSize(unsigned int blockHeight)
{
    clear();
...
uint64_t TesraDGP::getMinGasPrice(unsigned int blockHeight)
{
    clear();
...
uint64_t TesraDGP::getBlockGasLimit(unsigned int blockHeight)
{
    clear();
...
bool TesraDGP::initStorages(const dev::Address &addr, unsigned int blockHeight, std::vector<unsigned char> data)
{
    initStorageDGP(addr);
...
void TesraDGP::initStorageDGP(const dev::Address &addr)
{
    storageDGP = state->storage(addr);
}

void TesraDGP::initStorageTemplate(const dev::Address &addr)
{
...
void TesraDGP::initDataTemplate(const dev::Address &addr, std::vector<unsigned char> &data)
{
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void TesraDGP::createParamsInstance()
{
...
dev::Address TesraDGP::getAddressForBlock(unsigned int blockHeight)
{
...
void TesraDGP::parseStorageScheduleContract(std::vector<uint32_t> &uint32Values)
{
...
void TesraDGP::parseDataScheduleContract(std::vector<uint32_t> &uint32Values)
{
...
void TesraDGP::parseStorageOneUint64(uint64_t &value)
{
...
void TesraDGP::parseDataOneUint64(uint64_t &value)
{
...
dev::eth::EVMSchedule TesraDGP::createEVMSchedule()
{
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
