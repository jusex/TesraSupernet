...
static FILE* LZ4IO_openDstFile(const char* dstFileName)
{
    ...
    if (!strcmp (dstFileName, stdoutmark)) {
        ...
        if (!g_overwrite && strcmp (dstFileName, nulmark)) {  
...
int LZ4IO_compressFilename_Legacy(const char* input_filename, const char* output_filename, int compressionlevel)
{
    ...
    while (1) {
        ...
        if (inSize == 0) break;
        if (inSize > LEGACY_BLOCKSIZE) EXM_THROW(23, "Read error : wrong fread() size report ");   
        filesize += inSize;
    ...
    clockEnd = clock();
    if (clockEnd==clockStart) clockEnd+=1;  
    filesize += !filesize;   
...
static cRess_t LZ4IO_createCResources(void)
{
    ...
    ress.dstBufferSize = LZ4F_compressFrameBound(blockSize, NULL);   
...
int LZ4IO_compressMultipleFilenames(const char** inFileNamesTable, int ifntSize, const char* suffix, int compressionLevel)
{
    ...
    if (dstFileName == NULL) return ifntSize;   
