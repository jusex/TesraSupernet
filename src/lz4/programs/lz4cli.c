...
int main(int argc, const char** argv)
{
    ...
    for(i=1; i<argc; i++) {
        ...
        if (!all_arguments_are_files && argument[0]=='-') {
            ...
            if (argument[1]=='-') {
                ...
                if (!strcmp(argument,  "--keep")) { LZ4IO_setRemoveSrcFile(0); continue; }   
            ...
            while (argument[1]!=0) {
                ...
                if (g_lz4c_legacy_commands) {
                    ...
                    if (!strcmp(argument,  "c0")) { cLevel=0; argument++; continue; }  
                    if (!strcmp(argument,  "c1")) { cLevel=9; argument++; continue; }  
                    if (!strcmp(argument,  "c2")) { cLevel=12; argument++; continue; } 
                    if (!strcmp(argument,  "hc")) { cLevel=12; argument++; continue; } 
                    if (!strcmp(argument,  "y"))  { LZ4IO_setOverwrite(1); continue; } 
                }
                ...
                case 'r': recursive=1;
                    ...
                    /* Treat non-option args as input files.  See https:
    ...
    while ((!output_filename) && (multiple_inputs==0)) {
        if (!IS_CONSOLE(stdout)) { output_filename=stdoutmark; break; }   
        if (mode == om_auto) {  
