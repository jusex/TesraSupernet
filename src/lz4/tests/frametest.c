...
#define COMPRESSIBLE_NOISE_LENGTH (2 MB)
    ...
    {   size_t const dictSize = 63 KB;
        ...
        if (cSizeWithDict >= cSizeNoDict) goto _output_error;  
...
int fuzzerTests(U32 seed, unsigned nbTests, unsigned startTest, double compressibility, U32 duration_s)
{
    ...
    unsigned testNb = 0;
    size_t const srcDataLength = 9 MB;  
    void* srcBuffer = NULL;
...
#   define CHECK(cond, ...) if (cond) { DISPLAY("Error => "); DISPLAY(__VA_ARGS__); \
    ...
    decodedBuffer = calloc(1, srcDataLength);   
    ...
    for ( ; (testNb < nbTests) || (clockDuration > FUZ_GetClockSpan(startClock)) ; testNb++) {
        ...
        {   const BYTE* ip = (const BYTE*)compressedBuffer;
            ...
            unsigned const nonContiguousDst = FUZ_rand(&randState) % 3;   
            size_t totalOut = 0;
            XXH64_state_t xxh64;
            XXH64_reset(&xxh64, 1);
            while (ip < iend) {
                ...
                ip += iSize;
                op += nonContiguousDst;
                if (nonContiguousDst==2) op = (BYTE*)decodedBuffer;   
            }
