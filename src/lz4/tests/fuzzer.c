...
#   define FUZ_DISPLAYTEST(...) {                 \
    ...
    for (cycleNb = startCycle;
        ...
        {   LZ4_stream_t LZ4_stream;
            ...
            LZ4_compress_fast_continue (&LZ4_stream, dict, compressedBuffer, dictSize, (int)compressedBufferSize, 1);   
...
static void FUZ_unitTests(int compressionLevel)
{
    ...
    {   LZ4_streamHC_t* sp;
        ...
        {   XXH64_state_t xxhOrig;
            ...
            char* const ringBufferFast = testVerify + dBufferSize + 1;   
            int iNext = 0;
            int dNext = 0;
            int compressedSize;

            assert((size_t)(dBufferSize + 1 + dBufferSize) < testVerifySize);   
            XXH64_reset(&xxhOrig, 0);
            XXH64_reset(&xxhNewSafe, 0);
            XXH64_reset(&xxhNewFast, 0);
            LZ4_resetStreamHC(&sHC, compressionLevel);
...
#define BSIZE1 (dBufferSize - (maxMessageSize-1))
            ...
            iNext = BSIZE1+1;
            assert(BSIZE1 >= 65535);
            memcpy(testInput + iNext, testInput + (BSIZE1-65535), messageSize);  
            ...
            while (totalMessageSize < 9 MB) {
                ...
                result = LZ4_decompress_safe_continue(&decodeStateSafe,
                                                      ...
                                                      compressedSize, dBufferSize - dNext);   
